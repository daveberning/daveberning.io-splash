  ...
  return function (t) {
    ...
    0: function (t, e, i) {
      ...
      class n extends s.b {
        ...
        genPoint(t, e, i) {
          ...
          if (this.points || (this.points = []), this.options.showDots) {
            const t = new THREE.SphereGeometry(.25, 12, 12),
              e = new THREE.MeshLambertMaterial({color: this.options.color})
        ...
        onInit() {
          this.cont = new THREE.Group, this.cont.position.set(0, 0, 0), this.scene.add(this.cont)
          ...
          r.addAttribute('position', new THREE.BufferAttribute(this.linePositions, 3).setDynamic(!0)), r.addAttribute('color', new THREE.BufferAttribute(this.lineColors, 3).setDynamic(!0)), r.computeBoundingSphere(), r.setDrawRange(0, 0)
          const h = new THREE.LineBasicMaterial({
            vertexColors: THREE.VertexColors,
            blending: 'additive' === this.blending ? THREE.AdditiveBlending : null,
            transparent: !0
          })
          this.linesMesh = new THREE.LineSegments(r, h), this.cont.add(this.linesMesh)
          ...
          this.camera = new THREE.PerspectiveCamera(25, this.width / this.height, .01, 1e4), this.camera.position.set(50, 100, 150), this.scene.add(this.camera)
          const a = new THREE.AmbientLight(16777215, .75)
          return this.scene.add(a), this.spot = new THREE.SpotLight(16777215, 1), this.spot.position.set(0, 200, 0), this.spot.distance = 400, this.spot.target = this.cont, this.scene.add(this.spot)
        }
        ...
        onUpdate() {
          ...
          let i = 0, s = 0, o = 0
          const n = new THREE.Color(this.options.backgroundColor), r = new THREE.Color(this.options.color),
            h = r.clone().sub(n)
          ...
          for (let t = 0; t < this.points.length; t++) {
            ...
            for (let c = t; c < this.points.length; c++) {
              const t = this.points[c], u = e.position.x - t.position.x, p = e.position.y - t.position.y,
              ...
              if ((a = Math.sqrt(u * u + p * p + d * d)) < this.options.maxDistance) {
                ...
                l = 'additive' === this.blending ? new THREE.Color(0).lerp(h, c) : n.clone().lerp(r, c), this.linePositions[i++] = e.position.x, this.linePositions[i++] = e.position.y, this.linePositions[i++] = e.position.z, this.linePositions[i++] = t.position.x, this.linePositions[i++] = t.position.y, this.linePositions[i++] = t.position.z, this.lineColors[s++] = l.r, this.lineColors[s++] = l.g, this.lineColors[s++] = l.b, this.lineColors[s++] = l.r, this.lineColors[s++] = l.g, this.lineColors[s++] = l.b, o++
              }
            }
          }
        ...
        onMouseMove(t, e) {
          ...
          return i.tz = o * Math.sin(n), i.tx = o * Math.cos(n), i.ty = i.oy + 10 * (e - .5) * (this.options.mouseCoeffY || 1), this.rayCaster, this.rcMouseX = 2 * t - 1, this.rcMouseY = 2 * -t + 1
        }
      ...
      n.initClass(), e.default = s.a.register('NET', n)
    }
  })
})